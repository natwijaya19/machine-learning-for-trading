...
# import njit decorator from numba
...
def kalman_filter_looping(kf: KalmanFilter, data: ndarray) -> List[float]:
    """
    Loop through prices and apply Kalman filter.

    Parameters:
        kf (KalmanFilter): Kalman filter object.
        data (ndarray): data to filter.
    """
    filtered_state_means: List[float] = []
    t: tp.Union[int, None] = None
    filtered_state_mean: tp.Union[ndarray, None] = None
    for t in range(len(data)):
        filtered_state_mean, _ = kf.kalman_filter_step(z=data[t])
        filtered_state_means.append(float(filtered_state_mean))
    return filtered_state_means


def plot_data(filtered_data: DataFrame, ticker: str) -> None:
    """
...
def main():
    ...
    filtered_state_means: list[float] = kalman_filter_looping(kf, prices)
    ...
    # Plot the original and filtered prices
    plot_data(filtered_data, ticker)
